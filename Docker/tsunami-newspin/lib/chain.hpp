    ...
    void twobody_interaction_vdep(size_t k, size_t j, const double3 &dr, const double3 &dv, double *mass,
        ...
        if (Config.wPNs) {
            if (v2 > vel_light2) {
                ...
                if constexpr (withspin and false) {
                    //spin equations will be deifned in terms of "dimensional" spins
                    // because it requires fewer flops; for reference S = G*m*m*chi
                    // where S is the dimensional spin, m is the mass, and chi is the dimensionless spin
                    ...
                    if (Config.pn1 and Config.so) {
                        ...
                        t1soj = (murinv3 * (2 + 1.5 * mk / mj)) * Ln.cross(sj);
                        t1sok = (murinv3 * (2 + 1.5 * mj / mk)) * Ln.cross(sk);
                    ...
                    if (Config.pn35 and Config.so) {
                        ...
                        double v2rdot2 = v2 * rdot2;
                        double v2Ut = v2 * Ut;
                        double rdot2Ut = rdot2 * Ut;
                        double3 term35so2 = .5 * (
                                stot *
                                (31 * v4 - 260 * v2rdot2 + 245 * rdot4 - c689_3 * v2Ut + 537 * rdot2Ut + c4_3 * Ut2)
                                - xi * (29 * v4 - 40 * v2rdot2 - 245 * rdot4 + 211 * v2Ut - 1019 * rdot2Ut - 80 * Ut2)
                        );
                    ...
                    acc += acc_spin;
                    double3 tj = coeff1pn*(t1ssj + t1sok) + coeff35pn*t35ssj;
                    double3 tk = coeff1pn*(t1ssk + t1soj) + coeff35pn*t35ssk;
