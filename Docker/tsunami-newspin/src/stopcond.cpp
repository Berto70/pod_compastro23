...
bool StopCond_Triple::check_stopping_condition(ch_real3 *pos, ch_real3 *vel, ch_real *mass, ch_real *rad, ch_real time) {
    ...
    if (not iBound.empty()) {
        ...
        ch_real3 dv = PairInner.vcom - vel[k];
        ch_real3 dp = PairInner.pcom - pos[k];
        ...
        if (PairOuter.a < 0.0) {
            ...
            if (PairOuter.vdotr > 0.0) {
                if (not is_escaping) {
                    ...
                    ch_real3 dout = PairInner.pcom - pos[k];
...
bool StopCond_TripleTDEs::check_stopping_condition(ch_real3 *pos, ch_real3 *vel, ch_real *mass, ch_real *rad, ch_real time) {
    for (size_t i=0; i<3; i++) {
        if (i == star_id) continue;

        // Record PTDEs : R_t < R < 2 r_t
        ch_real3 dp = pos[i] - pos[star_id];
