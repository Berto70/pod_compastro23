...
void KeplerUtils::kepl_to_cart(double out_posvel[6], double prim_posvel[6], double mu, double a, double e, double i, double ome, double Ome, double nu){
    if(e == 1.){
        throw TsunamiError("e == 1", __FILE__, __LINE__);
    }
    if(e < 0.){
        throw TsunamiError("e < 0", __FILE__, __LINE__);
    }
    if(e > 1.){
        if(a > 0.){
            throw TsunamiError("e > 1 but a > 0", __FILE__, __LINE__);
        }
    }
    else{
        if(a < 0.){
    ...
    if(e*cos(nu) < -1.){
        throw TsunamiError("e*cos(nu) < -1", __FILE__, __LINE__);
    }
...
void KeplerUtils::cart_to_kepl(double out_keplpar[6], double in_posvel[6], double mu) {
    ...
    if(mu < dtiny) {
        throw TsunamiError("total mass is zero", __FILE__, __LINE__);
    }
    ...
    if(r < dtiny) {
        throw TsunamiError("particle have same position", __FILE__, __LINE__);
    }
