...
def setup_system(m1=1e6, m2=1e5, a1=100, e1=0.0, ome1=0.0, M1=0.0, Rschw_mult=1,
    ...
    mout = m1+m2+m3
    pos_vel3 = KU.kepl_to_cart(pos_vel_inn_com, minn, m3, a2, e2, i2, ome2, Ome2, nu2)
...
def run_system(p, v, m, R, ft=1000, dt_out=0.5, stop_dist=10000, save_trajectories=True, fname="0000000"):
    ...
    #code.dcoll = 0.0
    # Particle types. Must be 1 for star, 14 for BHs
    st = np.array([14, 14, 1])
    idstar = 2
    ...
    Psi = Psi * (0.80 + 0.26 * ((m[:2]*1e-6) )**0.5)
    Rt_full = Rt * Psi
    Rt_part = 2*Rt
    collision_type = "None"
    ...
    while time < ftime:
        ...
        if code.check_collision() > 0:
            ...
            dp = p[id1] - p[id2]
            dv = v[id1] - v[id2]
            orb = KU.cart_to_kepl(dp, dv, m[id1], m[id2])
            bhid = id1 if id1 != idstar else id2
            peri = orb[0] * (1 - orb[1])
            if peri < R[bhid]:
                collision_type = "Direct plunge"
            else:
                collision_type = "Full TDE"
            break
    ...
    PTDELog = code.get_PTDELog()
    
    print("Event type:", collision_type, end="\033[K\n")
    print("Partial TDE:", PTDELog.had_ptde)
    
    for i, istar in enumerate(PTDELog.stars_ind):
        print("Checking PTDEs of star", istar)
        PTDEHist = PTDELog.PTDEHistories[istar]
        Nevents = len(PTDEHist.PTDE_list)
        print("Nevents", Nevents)
        for i, PTDe in enumerate(PTDEHist.PTDE_list):
            print("PTDE #{:d}".format(i))
            print("semi", PTDe.semi)
            print("ecc", PTDe.ecc)
            print("bh index", PTDe.ibh)
            print("rmin", PTDe.rmin)
...
def escape_condition(m, p, v, stop_dist=10000, it=2):
    ...
    for i in [0,1]:
        ...
        orb = KU.cart_to_kepl(dp, dv, m[i], m[it])
    ...
    orb = KU.cart_to_kepl(dp, dv, mb, m[2])
    is_bound = is_bound or (orb[0] > 0)
    dist = (dp*dp).sum()**0.5
...
def option_parser():
    ...
    result.add_argument("-M1", help="SMBH binary mean anomaly, degrees",
                        dest="M1", type=float, default=53.0)
