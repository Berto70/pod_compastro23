...
pdf = matplotlib.backends.backend_pdf.PdfPages(path + "array_tot.pdf")

# Generate some sample data
fig, ax = plt.subplots(nrows=2, ncols=1, figsize=(10, 15))

# Main plot
for i in range(array_tot.shape[0]):
    ax[0].plot(N_tot, array_tot[i], 'o-', label=func_list[i])
    ax[1].plot(N_tot, array_tot[i], 'o-', label=func_list[i])
...
for i in range(array_tot.shape[0]):
    axins.plot(N_tot, array_tot[i], 'o-', label=func_list[i])
