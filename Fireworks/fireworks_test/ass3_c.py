...
while t < 10*Tperiod:
    part, _, acc, _, _ = fint.integrator_rk4(part, tstep=0.01, acceleration_estimator=fdyn.acceleration_pyfalcon)
...
ax[1].set_ylabel('v_x')
ax[1].legend()
ax[1].set_title('Velocity on Z Plane')
plt.savefig('./fireworks_test/plots/velocity_plot.pdf')
...
plt.savefig('./fireworks_test/plots/peri_apo.pdf')
